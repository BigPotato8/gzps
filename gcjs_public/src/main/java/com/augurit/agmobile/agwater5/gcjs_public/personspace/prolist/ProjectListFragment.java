...
public class ProjectListFragment extends BaseViewListFragment <ProjectDetailBean.ProjectInfoBean>{
...
//        params.put("localCode", keyword);
        params.put("pageNum", page + "");
        params.put("pageSize", rows + "");

        return HttpUtil.getInstance(AwUrlManager.serverUrl()).get(GcjsPuUrlConstant.APPROVE_LIST, params).map(
                s -> {

                    MyProjectBean result = new Gson().fromJson(s, new TypeToken<MyProjectBean>() {
                    }.getType());

                    if (!result.isSuccess()) {
                        ToastUtil.shortToast(getActivity(),result.getMessage());
                        return new ApiResult<>();
                    }
                    ApiResult<List<ProjectDetailBean.ProjectInfoBean>> apiResult = new ApiResult<>();
                    ApiResult<ProjectDetailBean.ProjectInfoBean> data =  new ApiResult<>();
                    apiResult.setData(result.getContent().getList());
                    return apiResult;
                }
        );
    }
