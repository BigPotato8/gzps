...
import com.augurit.common.common.util.SharedPreferencesUtil;
...
//        progressBar = (ProgressDialog) findViewById(R.id.loading);
        progressBar = new ProgressDialog(this);
        progressBar.setCancelable(false);
        progressBar.setMessage("正在下载...");
        progressBar.setButton(DialogInterface.BUTTON_NEGATIVE, "取消", (dialog, which) -> {
            AuthDownloadManager.getInstance().release();
            progressBar.dismiss();
            ToastUtil.shortToast(this, "已取消下载");
        });
        progressBar.show();
    ...
    private class MyViewPagerAdpapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            SubsamplingScaleImageView iv_run = view.findViewById(R.id.iv_run);
            PDFView pdfView = view.findViewById(R.id.pdfView);
            String mPath = mData.get(position);
            if (!mPath.toLowerCase().contains(".pdf")
                    && !mPath.toLowerCase().contains(".doc") && !mPath.toLowerCase().contains(".xls")) {
...
//            super.destroyItem(container, position, object);
    ...
    public void downFile(String mPath, Context ctx) {
        ...
        //判断之前是否下载成功
        boolean isDownload = mSharedPreferencesUtil.getBoolean(mPath, false);

        if (file.exists() && file.isFile() && isDownload) {
            progressBar.dismiss();
            //直接打开文件
            previewFile(fileName, dPath, ctx);
        } else {
//            //下载文件，并打开文件
////            ProgressDialogUtil.showProgressDialog(ctx, "正在下载...", true, false);
////            cancelDownListrener(ctx);
            ...
            AuthDownloadManager.getInstance()
                    ...
                    .Download(new AuthDownloadListener() {
                        ...
                        public void success() {
                            ...
                            mSharedPreferencesUtil.setBoolean(mPath, true);
    ...
    private void downloadFile(String mPath, String downloadPath, String fileName) {
        EasyHttp.downLoad(mPath)
    ...
    private void cancelDownListrener(Context ctx) {

        //点击返回时停止下载
        ProgressDialogUtil.setDismissListener(new DialogInterface.OnDismissListener() {
            @Override
            public void onDismiss(DialogInterface progressDialog) {
                if (downloadSuccess) {//下载成功
                    return;
                }
                DialogUtil.MessageBox(WatchImageOrPdfActivity.this, "停止下载", "是否停止下载", (dialog, which) -> {
                            dialog.dismiss();
...
//        }
        ...
        if (name.contains("policy")) {
            ...
            name = "改革流程图";
        } else if (name.contains("item_mat_list")) {
            name = "事项清单";
        }
