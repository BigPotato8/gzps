...
function sdasdasdas(data) {
	...
	$("#tabTitle").html(time+"月份全市各区农村生活污水治理项目运行维护管理监督考核结果");
...
var Table=function(){
	...
	Table.Init=function(){
		$("#table").bootstrapTable({
			...
			toggle:"tableCell",
			url:'http://139.159.232.250:8080/agsupport_swj/rest/reportForm/getDistricAssessResultList',//
			rowStyle:"rowStyle",
			cache:false,
			pagination:true,
			dataType:'jsonp',
			...
			columns:[
				[
					...
					{field:'DISTRICT_NAME',title:'行政区/单位',visible:true,align:'center',formatter: function (value, row, index) {
							if(value=="林业和园林(农污)"){
								return "林场";
							}else{
					...
					{field:'ASSESSNUM',title:'行政村（社区）<br>总数',visible:true,align:'center'},
					{field:'DISTRICTSCORE',title:'分散式考核行政村<br>（社区）平均得分',visible:true,align:'center'},
					{field:'VILLAGESCORE',title:'区级考核<br>得分',visible:true,align:'center'},
...
function  exportExcle() {
	...
	window.open( '/agsupport_swj/rest/reportForm/getDistricAssessResultList?queryTime="'+queryTime+'"&export=true&appRanking=false');
}
