                ...
                AuthDownloadManager.getInstance()
                        ...
                        .Download(new AuthDownloadListener() {
                            ...
                            public void failed() {
                                //跳出对话框，提示无证书
                                ProgressDialogUtil.dismissProgressDialog();
                                ToastUtil.shortToast(OtherResultAddActivity.this, "文件下载失败");
                            }
                        });
            }
        } else {
    ...
    protected void onFormLoaded() {
        ...
        title_bar.setMoreButtonAction(v -> {
            if (mData != null) {
                if (!isEdit) {
                    isEdit = true;
                    title_bar.setTitleText("编辑其它结果物");
                                        ...
                                        .subscribe(s -> {
                                            if (!TextUtils.isEmpty(s) && StringUtils.isJson(s)) {
                                                ...
                                                if (apiResult != null && apiResult.isSuccess()) {
                                                    ToastUtil.shortToast(OtherResultAddActivity.this, "删除成功");
...
//                                                    mFormPresenter.setWidgetValue("file", mFileBeans);
                                                    ...
                                                    ToastUtil.shortToast(OtherResultAddActivity.this, "删除失败");
                                                }
                                            }
                    ...
                    .subscribe(s -> {
                        if (progressDialog.isShowing()) {
                            ...
                            if (!TextUtils.isEmpty(s) && StringUtils.isJson(s)) {
                                ...
                                if (apiResult != null && apiResult.isSuccess()) {
                                    ToastUtil.shortToast(OtherResultAddActivity.this, "提交成功");
                                    setResult(RESULT_OK);
                                    finish();
                                } else {
                                    ToastUtil.shortToast(OtherResultAddActivity.this, "提交失败");
                                }
                            }
                        }
                    }, throwable -> {
                        throwable.printStackTrace();
                        ToastUtil.shortToast(OtherResultAddActivity.this, "网络错误，提交失败");
                    });
