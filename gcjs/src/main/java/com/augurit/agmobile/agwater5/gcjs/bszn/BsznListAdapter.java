 ...
 * @modifyMemo 修改备注：
 */
public class BsznListAdapter extends BaseViewListAdapter<String> {

    protected int mEventState = EventState.HANDLING;

    protected IDictRepository mDictRepository;

    public BsznListAdapter(Context context) {
        super(context);
        mDictRepository = AgWaterInjection.provideDictRepository(context);
    }

    public BsznListAdapter(Context context, int state) {
        super(context);
        mDictRepository = AgWaterInjection.provideDictRepository(context);
...
//    }
    ...
    public BaseDataViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.gcjs_bszn_list_item, parent, false);
