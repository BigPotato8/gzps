 ...
 * @modifyMemo 修改备注：
 */
public class ApproveOtherResultAdapter extends BaseQuickAdapter<PwpfBean.ItemMatinstBean, BaseViewHolder> {
    private FileDeleteListener deleteListener;

    public ApproveOtherResultAdapter(int layoutResId) {
        super(layoutResId);
    }

    @Override
    protected void convert(BaseViewHolder helper, PwpfBean.ItemMatinstBean item) {
        ...
        helper.setText(R.id.tv_title, item.getOfficialDocTitle());
        helper.setText(R.id.tv_create_user, item.getCreator());
        ...
        if (!ListUtil.isEmpty(item.getAttFiles())) {
            ApproveFileAdapter adapter = new ApproveFileAdapter(R.layout.layout_approve_file_item, item.getAttFiles());
            recyclerView.setAdapter(adapter);
            adapter.setOnItemClickListener((adapter1, view, position) -> {
                PwpfBean.ItemMatinstBean.AttFilesBean attFilesBean = (PwpfBean.ItemMatinstBean.AttFilesBean) adapter1.getItem(position);
